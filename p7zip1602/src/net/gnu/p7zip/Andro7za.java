...
public final class Andro7za {
	...
	private String PRIVATE_PATH = "/data/data/net.gnu.p7zip";
	//public static String zpaqx86 = PRIVATE_PATH + "/zpaq-x86";
	private String zpaqarm = PRIVATE_PATH + "/7z-armeabi-v7a";
	...
	public Andro7za(final Context ctx, final String tempDir) {
		Log.d(TAG, "PackageName: " + ctx.getPackageName());
		PRIVATE_PATH = "/data/data/" + ctx.getPackageName();
		zpaqarm = PRIVATE_PATH + "/7z-armeabi-v7a";
		
		if (tempDir == null || !new File(tempDir).mkdirs()) {
			final String files = ctx.getExternalFilesDir("7z").getAbsolutePath();
			mOutfile = files + "/7zOut.txt";
			mInfile = files + "/7zIn.txt";
			listFile = files + "/7zFileList.txt";
		} else {
			mOutfile = tempDir + "/7zOut.txt";
			mInfile = tempDir + "/7zIn.txt";
			listFile = tempDir + "/7zFileList.txt";
		}
...
//			}
			...
			if (!new File(zpaqarm).exists()) {
				FileUtil.is2File(ctx.getAssets().open("7z-armeabi-v7a"), zpaqarm);
			...
			command = new Command(zpaqarm, "a", "/sdcard/zpaq3.7z", "/sdcard/zpaq");
			command.setCommandListener(new CommandListener(command));
			command.run();
	...
	throws IOException {
		...
		Command command = new Command(otherArgs);
	...
	public int extractInEx(
		...
		Command command = new Command(otherArgs);//p7z + " " + cmd + " " + zArchive + " " + " -bb " + " " + password + " " + overwriteMode + " " +  pathToExtract + " " + otherArgs + includesTmp2 + excludesTmp2);
		Log.d(TAG, "command: " + command);
		CommandListener commandListener = new CommandListener(command, update);
		command.setCommandListener(commandListener);
		command.run();
